...
class LeafSeverityCalculator(toga.App):
    def startup(self):
        ...
        self.logo = toga.ImageView(
            toga.Image("resources/logo.png"),  # Cambia a la ruta real de tu logo
            style=Pack(height=100, padding_top=20, padding_bottom=10, alignment=CENTER)
        )
        main_box.add(self.logo)
    ...
    async def take_photo(self, widget, **kwargs):
        ...
        try:
            ...
            await self.main_window.dialog(
                toga.InfoDialog("Oh no!", "The Camera API is not implemented on this platform")
            )
        except PermissionError:
            await self.main_window.dialog(
                toga.InfoDialog("Oh no!", "You have not granted permission to take photos")
            )
    ...
    async def procesar_imagen(self, widget, **kwargs):
        ...
        try:
            ...
            await self.main_window.dialog(
                toga.InfoDialog("Error", f"Error al procesar la imagen: {str(e)}")
            )
        finally:
            self.processing = False
    ...
    async def open_image(self, widget, **kwargs):
        fb = UriFileBrowser()
        urilist = await fb.open_file_dialog("Seleccionar imagen", file_types=["jpg", "png"])
...
def main():
    return LeafSeverityCalculator("LeafSeverityCalculator", "com.leafseverity.app")
